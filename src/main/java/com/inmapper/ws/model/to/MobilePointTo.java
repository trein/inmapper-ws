...
public class MobilePointTo {
    ...
    /*
     * Whatever sensors you use, the X, Y, Z that you're measuring will depend
     * on the orientation of the device. However, for detecting the activities
     * that you mention, the result can't depend on e.g. whether the user is
     * holding the device in a portrait or landscape position, or whether the
     * device is flat or vertical, so the individual values of X, Y and Z won't
     * be any use. Instead you'll have to look at the length of the vector, i.e.
     * sqrt(X*X+Y*Y+Z*Z) which is independent of the device orientation.
     */
    @JsonIgnore
    ...
    public String toString() {
        return Objects.toStringHelper(this).add("x", this.x).add("y", this.y).add("z", this.z)
                .add("heading", this.heading).toString();
    }
    ...
    public static Double computeStandardDeviation(List<MobilePointTo> inWindowPoints) {
        Double average = computeAverage(inWindowPoints);
        Double variationSum = 0.0;
        for (MobilePointTo point : inWindowPoints) {
            variationSum += Math.pow((average - point.getVariation().doubleValue()), 2);
        }
        return Math.sqrt(variationSum / inWindowPoints.size());
    }
