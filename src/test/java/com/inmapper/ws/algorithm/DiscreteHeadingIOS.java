...
public class DiscreteHeadingIOS {
    ...
    public void test5Step1() throws InvalidMobilePositionException {
        ...
        mobilePoints = mobileSessionTo.getPositions();
        Double calibrationPhi = mobilePoints.get(0).getHeading();
        for (MobilePointTo mobilePointTo : mobilePoints) {
            if (mobilePointTo.getHeading() - calibrationPhi >= 45 && mobilePointTo.getHeading() - calibrationPhi < 135) {
                mobilePointsDiscrete.add(new MobilePointTo(mobilePointTo.getX(), mobilePointTo.getY(), mobilePointTo
                        .getZ(), Double.valueOf(90)));
            } else if (mobilePointTo.getHeading() - calibrationPhi >= 135
                    && mobilePointTo.getHeading() - calibrationPhi < 225) {
                mobilePointsDiscrete.add(new MobilePointTo(mobilePointTo.getX(), mobilePointTo.getY(), mobilePointTo
                        .getZ(), Double.valueOf(180)));
            } else if (mobilePointTo.getHeading() - calibrationPhi >= 225
                    || mobilePointTo.getHeading() - calibrationPhi < 315) {
                mobilePointsDiscrete.add(new MobilePointTo(mobilePointTo.getX(), mobilePointTo.getY(), mobilePointTo
                        .getZ(), Double.valueOf(270)));
            } else {
                mobilePointsDiscrete.add(new MobilePointTo(mobilePointTo.getX(), mobilePointTo.getY(), mobilePointTo
                        .getZ(), Double.valueOf(0)));
            }
        }
        System.out.println(mobilePointsDiscrete.size());
        for (MobilePointTo mobilePointTo : mobilePointsDiscrete) {
